...
from controller.DefWindowController import DefWindowController
...
class Ui_DefWindow(object):
    def setupUi(self, MainWindow, settings=None, viewType=ViewType.VIEW, defin=Defins.NORMALIZATION):
        ...
        self.__definDict = None
        self.__definRefs = None
        self.__vectVals = None
        self.__clustVals = None
        self.__pos = None

        self.__initDefinDict()
        self.__initDefinRefs()
        self.__initVectVals()
        self.__initClustVals()
        self.__initPos()

        self.__notEditableDefs = [Defins.NORMALIZATION, Defins.FILTER, Defins.PREP, Defins.REDUCEDIM, Defins.PREPMETH,
                                  Defins.CLEARDATA]
        self.__usableDefs = [Defins.LEM, Defins.STEM, Defins.SW, Defins.TOKEN, Defins.NGRAMM]
        self.__selectableDefs = [Defins.CLUST, Defins.VECTORIZATION, Defins.LEM]
        self.__inputDefs = [Defins.TOKEN, Defins.SW, Defins.NGRAMM]
        self.__onlyUse = [Defins.STEM]
        ...
        self.TEDef.setText(self.__definDict[defin])
        ...
        if self.defin in self.__onlyUse:
            self.LVal.hide()
            self.LEVal.hide()

        self.__controller = DefWindowController(settings)
    ...
    def setupAll(self):
        self.setupRefs()
        if self.defin in self.__notEditableDefs:
            self.setupNotEditableFields()
        self.setupButtons()
        if self.defin in self.__selectableDefs:
            self.setupCBVal()
        if self.defin in self.__inputDefs:
            self.setupTEVal()
        if self.defin not in self.__usableDefs:
            self.CBUse.hide()
        else:
            self.checkDef()
    ...
    def setupRefs(self):
        ...
        item = QtWidgets.QTableWidgetItem()
        self.TWRefs.setHorizontalHeaderItem(1, item)
        self.TWRefs.setRowCount(len(self.__definRefs[self.defin]))
        ...
        for i, ref in enumerate(self.__definRefs[self.defin]):
            item = QtWidgets.QTableWidgetItem(ref)
            item.setToolTip(ref)
            self.TWRefs.setItem(i, 0, item)
            self.TWRefs.setItem(i, 1, QtWidgets.QTableWidgetItem(self.__definRefs[self.defin][ref]))
    ...
    def setupButtons(self):
        ...
        self.BAccept.clicked.connect(self.__saveParam)

        if self.defin == Defins.SW:
            self.newSW = []
            self.removeSW = []
            self.BAddSw.clicked.connect(self.__addSw)
            self.BRemoveSw.clicked.connect(self.__removeSw)
    ...
    def setupCBVal(self):
        ...
        if self.defin == Defins.VECTORIZATION:
            self.LEVal.hide()
            for el in self.__vectVals:
                self.CBVal.addItem(self.__vectVals[el], el)
        ...
        if self.defin == Defins.CLUST:
            ...
            for el in self.__clustVals:
                self.CBVal.addItem(self.__clustVals[el], el)
        ...
        if self.defin == Defins.LEM:
            ...
            for el in self.__pos:
                self.CBVal.addItem(self.__pos[el], el)
    ...
    def __saveParam(self):
        wndData = {
            "Defin": self.defin,
            "CBVal": self.CBVal.currentData(),
